...
---@param string table|nil
---@param char string delimiter that replaces spaces (- for titles, _ for tags)
...
function M.retitle(options, filename, title)
	...
	if options.retitle_heading and title ~= "" then
		...
		if
			(first_char == "#" and options.ext == "md")
			or (first_char == "*" and (options.ext == "org" or options.ext == "norg"))
		then
...
function M.retag(options, filename, new_tags)
	...
	new_tags = M.format_denote_string(new_tags, "_")
	new_filename = new_filename .. new_tags .. "." .. options.ext
	M.replace_file(filename, new_filename)
end
